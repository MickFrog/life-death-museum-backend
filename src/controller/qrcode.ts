...
qrcodeRouter.post(
  ...
  async (req: Request, res: Response, _next: NextFunction) => {
    try {
      ...
      const { buffer, mimeType } = ImageConverter.base64ToBuffer(captured_image_data);
      ...
      // Upload to storage
      const capturedImageUrl = await storage.uploadFromBuffer(buffer, storagePath, mimeType);
      ...
      const qrCodeImageUrl = await storage.uploadFromBuffer(qrCodeBuffer, qrCodeStoragePath, 'image/png');
